...
class ManualInspectionController {
  submitManualData = async (req, res) => {
    try {
      ...
      const compName = String(
        componentName || componentCategory || "General",
      ).trim();

      const userRole = bodyUserRole || req.user?.role;
      const isSuperAdmin =
        String(userRole || "")
          .toLowerCase()
          .includes("super") && !bodyCompanyId;

      // 1. Extract Company & User information (Multi-tenant Tracking)
      let companyId =
        bodyCompanyId || (isSuperAdmin ? null : req.user?.companyId || null);
      let companyName = bodyCompanyName || null;
      if (companyId && !companyName) {
        try {
          const compRec = await prisma.company.findUnique({
            where: { id: companyId },
            select: { name: true },
          });
      ...
      const userName =
        ...
        req.user?.name ||
        req.user?.firstName ||
        (isSuperAdmin ? "Super Admin" : "Company Staff");
      ...
      const finalSerialNumber =
        serialNumber ||
        `SN-${finalBrand.toUpperCase().substring(0, 4)}-${Math.abs(
          (machineId.split("").reduce((a, b) => {
      ...
      try {
        if (targetCompanyId) {
          ...
          if (resolvedMachine) {
            ...
            const existingIdCheck = await prisma.machine.findUnique({
              where: { id: machineId },
            });
            const generatedId = existingIdCheck
      ...
      if (isBatch) {
        ...
        for (const c of batchComponents) {
          ...
          try {
            ...
            if (prevRec) previousHealth = prevRec.healthScore; // NEW
          } catch (e) {}

          // Calculate component health
          const safeParams = JSON.parse(JSON.stringify(cParams));
          const cHealth = healthEngineService.calculateHealth(
            c.readings,
            c.checklist,
            safeParams,
          );
          ...
          if (prevParams && Array.isArray(prevParams)) {
            ...
            prevParams.forEach((p) => {
              if (p && p.name)
                prevMap.set(String(p.name).trim().toLowerCase(), p.value);
            });
            ...
            safeParams.forEach((curr) => {
              ...
              if (
                oldVal !== undefined &&
                oldVal !== null &&
                String(oldVal) !== String(curr.value)
              ) {
        ...
        const batchActionType = hasAnyChanges
          ? "ROUTINE_UPDATE"
          : "INITIAL_INSPECTION";
        ...
        await prisma.$transaction(async (tx) => {
          ...
          if (allCompRecords.length > 0) {
            const totalScore = allCompRecords.reduce(
              (sum, c) => sum + (c.healthScore || 0),
              0,
            );
            overallMachineHealth = Math.round(
              totalScore / allCompRecords.length,
            );

            const hasCritical = allCompRecords.some(
              (c) => c.status === "Critical" || (c.healthScore || 0) < 50,
            );
            const hasWarning = allCompRecords.some(
              (c) =>
                c.status === "Warning" ||
                ((c.healthScore || 0) >= 50 && (c.healthScore || 0) < 85),
            );
          ...
          auditLogRow = await tx.machineInspectionAuditLog.create({
            data: {
              ...
              parameterChanges:
                allParameterChanges.length > 0 ? allParameterChanges : null,
    ...
    processedComponents.map((comp) =>
        ...
        if (resolvedMachine?.assignedSupervisorId) {
        ...
        return responseHandler(
          ...
          HTTP_STATUS.OK,
          true,
          `Full machine inspection saved (${batchComponents.length} components). Machine Health: ${overallMachineHealth}% (${machineStatus})`,
          {
            actionType: batchActionType,
      ...
      const safeParams = JSON.parse(
        JSON.stringify(
          customFields && customFields.length > 0
            ? customFields
            : { readings, checklist, customFields },
        ),
      );

      const healthResult = healthEngineService.calculateHealth(
        readings,
        checklist,
        safeParams,
      );
      ...
      const actionType =
        isInitial || parameterChanges.length === 0
          ? "INITIAL_INSPECTION"
          : "ROUTINE_UPDATE";
      ...
      try {
        auditLogRow = await prisma.machineInspectionAuditLog.create({
          data: {
            ...
            parameterChanges:
              parameterChanges.length > 0 ? parameterChanges : null,
       ...
       await alertService.evaluate({
        ...
        componentName: compName,
        previousHealth,
        currentHealth: healthResult.healthScore,
        status: healthResult.status,
        parameterChanges,
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        `Inspection saved for ${compName}. Status: ${healthResult.status} (${healthResult.healthScore}%)`,
        {
          actionType,
          auditLog: auditLogRow,
          componentHealth: {
      ...
      return responseHandler(
        ...
        false,
        error.message || "Inspection submit error",
      );
    }
  };
  ...
  getManualData = async (req, res) => {
    try {
      ...
      for (const r of records) {
        const key = String(r.componentName || r.componentId || r.id)
          .toLowerCase()
          .trim();
      ...
      if (uniqueRecords.length > 0) {
        const totalScore = uniqueRecords.reduce(
          (sum, c) => sum + (c.healthScore || 0),
          0,
        );
        overallMachineHealth = Math.round(totalScore / uniqueRecords.length);

        const hasCritical = uniqueRecords.some(
          (c) => c.status === "Critical" || (c.healthScore || 0) < 50,
        );
        const hasWarning = uniqueRecords.some(
          (c) =>
            c.status === "Warning" ||
            ((c.healthScore || 0) >= 50 && (c.healthScore || 0) < 85),
        );
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Manual data fetched successfully",
        {
          machine: {
            id: machineId,
            healthScore: overallMachineHealth,
  ...
  getEquipmentTemplate = async (req, res) => {
    try {
      ...
      const template = await equipmentSpecMasterService.getSpecTemplate(
        equipmentType,
        modelName,
      );
      const baseComponents = Array.isArray(template.components)
        ? [...template.components]
        : [];
      ...
      if (targetCompanyId) {
        try {
          ...
          if (customComponentsFromDB && customComponentsFromDB.length > 0) {
            const targetModelLower = String(modelName || "")
              .toLowerCase()
              .trim();
            ...
            const matchedComps = customComponentsFromDB.filter((c) => {
              ...
              const cMachineId = String(c.machineId || "")
                .toLowerCase()
                .trim();
              const cDesc = String(c.description || "")
                .toLowerCase()
                .trim();
              const cMachineModel = String(c.machine?.model || "")
                .toLowerCase()
                .trim();
              const cMachineName = String(c.machine?.name || "")
                .toLowerCase()
                .trim();

              // Match by machineId
              if (targetMachineId && cMachineId === targetMachineId)
                return true;
              ...
              if (targetModelLower) {
                if (
                  cDesc &&
                  (targetModelLower.includes(cDesc) ||
                    cDesc.includes(targetModelLower))
                )
                  return true;
                if (
                  cMachineModel &&
                  (targetModelLower.includes(cMachineModel) ||
                    cMachineModel.includes(targetModelLower))
                )
                  return true;
                if (
                  cMachineName &&
                  (targetModelLower.includes(cMachineName) ||
                    cMachineName.includes(targetModelLower))
                )
                  return true;

                const modelKeywords = targetModelLower
                  .split(" ")
                  .filter((w) => w.length > 3);
                if (
                  modelKeywords.some(
                    (kw) =>
                      cDesc.includes(kw) ||
                      cMachineModel.includes(kw) ||
                      cMachineName.includes(kw),
                  )
                )
                  return true;
              }
            ...
            matchedComps.forEach((c) => {
              ...
              if (
                !baseComponents.some(
                  (bc) =>
                    (bc.name || "").toLowerCase() === compName.toLowerCase(),
                )
              ) {
                const rawParams = c.inspectionParameters || [];
                const parsedParams = Array.isArray(rawParams)
                  ? rawParams
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Equipment spec template fetched successfully",
        {
          ...template,
          companyId: targetCompanyId,
    ...
    try {
      ...
      if (!targetCompanyId) {
        return responseHandler(
          ...
          HTTP_STATUS.BAD_REQUEST,
          false,
          "Company ID is required to save custom component",
        );
      }
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        `Component "${compName}" successfully saved to company equipment`,
        existingComponent,
      );
    } catch (error) {
    ...
    try {
      ...
      if (machine) {
        return responseHandler(
          ...
          HTTP_STATUS.OK,
          true,
          `"${machine.name}" is already active in your company fleet`,
          {
            machine,
      ...
      try {
        const specTpl = await equipmentSpecMasterService.getSpecTemplate(
          category,
          effectiveModel,
        );
        if (
          specTpl &&
          Array.isArray(specTpl.components) &&
          specTpl.components.length > 0
        ) {
      ...
      return responseHandler(
        ...
        HTTP_STATUS.CREATED,
        true,
        `"${machine.name}" successfully added to your Company Fleet!`,
        {
          machine,
    ...
    try {
      ...
      try {
        const matchedCompany = await prisma.company.findFirst({
          where: {
            OR: [{ id: targetCompanyId }, { companyCode: targetCompanyId }],
          },
        });
        if (matchedCompany) {
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Company fleet machines fetched",
        machines || [],
      );
    } catch (err) {
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Company fleet machines (empty fallback)",
        [],
      );
    }
  };
    ...
    try {
      ...
      if (!targetMachine) {
        return responseHandler(
          ...
          false,
          "Machine not found in company fleet",
        );
      }

      // Cleanup related records
      await prisma.componentCost
        .deleteMany({ where: { component: { machineId: targetMachine.id } } })
        .catch(() => {});
      await prisma.componentHealth
        .deleteMany({ where: { machineId: targetMachine.id } })
        .catch(() => {});
      await prisma.failurePrediction
        .deleteMany({ where: { component: { machineId: targetMachine.id } } })
        .catch(() => {});
      await prisma.component
        .deleteMany({ where: { machineId: targetMachine.id } })
        .catch(() => {});
      await prisma.machineInspectionAuditLog
        .deleteMany({ where: { machineId: targetMachine.id } })
        .catch(() => {});
      await prisma.jobCard
        .deleteMany({ where: { machineId: targetMachine.id } })
        .catch(() => {});
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        `"${targetMachine.name}" has been removed from your Company Fleet`,
      );
    } catch (err) {
  ...
  getMasterCatalogFilters = async (req, res) => {
    try {
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Master equipment catalog filters fetched",
        filters,
      );
    } catch (error) {
      return responseHandler(
    ...
    try {
      ...
      if (!isExplicitPagination || isAllRequested) {
        ...
        const uniqueBrands = new Set(
          catalog.map((c) => c.brand).filter(Boolean),
        );
        const uniqueCategories = new Set(
          catalog.map((c) => c.category).filter(Boolean),
        );
        const totalSpecs = catalog.reduce(
          (sum, c) => sum + (Number(c.totalSpecsCount) || 0),
          0,
        );
        ...
        return responseHandler(
          ...
          HTTP_STATUS.OK,
          true,
          "Master equipment catalog fetched from database table",
          {
      ...
      return responseHandler(
        ...
        "Master equipment catalog fetched from database table",
        {
          ...
          totalSpecs: result.catalog.reduce(
            (sum, c) => sum + (Number(c.totalSpecsCount) || 0),
            0,
          ),
  ...
  getInspectionHistory = async (req, res) => {
    try {
      ...
      const isSuperAdmin = String(userRole || "")
        .toLowerCase()
        .includes("super");
      ...
      console.log(
        `[INSPECTION-HISTORY] fetching for user=${req.user?.email || "anon"} role=${userRole} company=${targetCompanyId}`,
      );
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Inspection history logs fetched successfully from database table",
        {
  ...
  reviewInspectionHistoryLog = async (req, res) => {
    try {
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Supervisor review recorded in database successfully",
        {
          log: updatedLog,
          supervisorRemarks,
          reviewRating,
    ...
    try {
      ...
      if (
        ...
        return responseHandler(
          ...
          HTTP_STATUS.OK,
          true,
          "Machine categories fetched successfully",
          [],
        );
      }
      ...
      fleetCatMap.forEach((info, catName) => {
        ...
        if (!exists) {
          ...
          const icon = lower.includes("crane")
            ? "Wrench"
            : lower.includes("truck") || lower.includes("dump")
              ? "Truck"
    ...
    try {
      ...
      if (
        ...
        return responseHandler(
          ...
          HTTP_STATUS.BAD_REQUEST,
          false,
          "Company ID is required to create an equipment type",
        );
      }
      ...
      if (existing) {
        return responseHandler(
          ...
          HTTP_STATUS.BAD_REQUEST,
          false,
          "An equipment type with this name already exists for your company",
        );
      }
  ...
  deleteInspectionHistoryLog = async (req, res) => {
    try {
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Inspection log record deleted successfully",
      );
    } catch (err) {
  ...
  getEquipmentTypes = async (req, res) => {
    try {
      ...
      return responseHandler(
        ...
        HTTP_STATUS.OK,
        true,
        "Equipment types fetched successfully",
        equipmentTypes,
      );
    } catch (err) {
