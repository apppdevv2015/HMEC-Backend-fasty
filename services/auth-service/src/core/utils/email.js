...
async function sendEmail(message) {
  const sendInline = String(process.env.EMAIL_SEND_INLINE || process.env.EMAIL_SEND_IMMEDIATE || "false").toLowerCase() === "true";
  ...
  setImmediate(() => {
    sendEmailNow(message)
      .then(() => {
        console.info("EMAIL_SENT", { to: message.to, subject: message.subject });
      })
      .catch((error) => {
        console.error("EMAIL_FAILED", { to: message.to, error: error.message });
      });
  });
